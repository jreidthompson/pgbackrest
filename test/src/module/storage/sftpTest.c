...
// libssh2 socket block directions
...
// libssh socket block directions
#define SSH_NO_BLOCK_READING_WRITING                                0
#define SSH_BLOCK_READING                                           2
#define SSH_BLOCK_WRITING                                           8
#define SSH_BLOCK_READING_WRITING                                   10
...
testRun(void)
{
    ...
    if (testBegin("storageNew()"))
    {
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh connect failure - no blocking");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh connect failure - block reading");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh connect failure - block writing");
        ...
        // Invalid hash type passed as 16 (a)
        TEST_ERROR(
            storageSftpNewP(
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[1,22]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_CONNECT, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_GET_SERVER_PUBLICKEY, .resultInt = SSH_ERROR},
            {.function = NULL}
        });
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh get ssh_get_fingerprint_hash - no prepended hash type");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_GET_SERVER_PUBLICKEY, .resultInt = SSH_OK, .resultZ = "server public key - unused"},
            {.function = HRNLIBSSH_GET_PUBLICKEY_HASH, .resultInt = SSH_OK, .resultZ = "public_key_hash"},
            {.function = HRNLIBSSH_GET_FINGERPRINT_HASH, .resultNull = false, .resultZ = "SHA256:fingerprint_hash"},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .hostFingerprint = STRDEF("fingerprint_hash")),
            ServiceError,
            "unable to init sftp session: sftp init error [2] sftp error [5]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost failure - strict - get server public key failure");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - get public key hash failure");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_CONNECT, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_GET_SERVER_PUBLICKEY, .resultInt = SSH_OK, .resultZ = "server public key"},
            {.function = HRNLIBSSH_GET_PUBLICKEY_HASH, .resultInt = SSH_ERROR},
            {.function = NULL}
        });
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - SSH_KNOWN_HOSTS_CHANGED");
        ...
        hrnCfgArgRawZ(argList, cfgOptRepoSftpHost, "localhost");
        hrnCfgArgRawZ(argList, cfgOptRepoSftpHostKeyHashType, "sha1");
        hrnCfgArgRawZ(argList, cfgOptRepoSftpPrivateKeyFile, TILDE_KEYPRIV_CSTR);
        hrnCfgArgRawZ(argList, cfgOptRepoSftpPublicKeyFile, TILDE_KEYPUB_CSTR);
        hrnCfgEnvKeyRawZ(cfgOptRepoSftpPrivateKeyPassphrase, 1, "keyPassphrase");
        HRN_CFG_LOAD(cfgCmdArchiveGet, argList);
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': mismatch in known hosts files: SSH_KNOWN_HOSTS_CHANGED [2]: check type [strict]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - SSH_KNOWN_HOSTS_ERROR");
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': known hosts error [-2]: check type [strict]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - accept-new - failure - SSH_KNOWN_HOSTS_ERROR");
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': known hosts error [-2]: check type [accept-new]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - SSH_KNOWN_HOSTS_OTHER");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,\"%%d/.ssh/known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,\"/etc/ssh/ssh_known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_OTHER},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,null]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,null]", .resultInt = SSH_OK},
            {.function = NULL}
        });
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': key type mismatch: SSH_SERVER_OTHER [3]: check type [strict]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - SSH_KNOWN_HOSTS_UNKNOWN");
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': server is unknown: SSH_KNOWN_HOSTS_UNKNOWN [0]: check type [strict]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - SSH_KNOWN_HOSTS_NOT_FOUND");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_GET_SERVER_PUBLICKEY, .resultInt = SSH_OK, .resultZ = "server public key"},
            {.function = HRNLIBSSH_GET_PUBLICKEY_HASH, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = -999},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,\"%%d/.ssh/known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,\"/etc/ssh/ssh_known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = -999},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,null]", .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': undefined failure [-999]: check type [strict]"
            );
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_GET_SERVER_PUBLICKEY, .resultInt = SSH_OK, .resultZ = "server public key"},
            {.function = HRNLIBSSH_GET_PUBLICKEY_HASH, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_NOT_FOUND},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,\"%%d/.ssh/known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,\"/etc/ssh/ssh_known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_NOT_FOUND},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,null]", .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "known hosts failure: 'localhost': could not find known hosts file: SSH_KNOWN_HOSTS_NOT_FOUND [-1]: check type [strict]"
            );

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - setting secondary known hosts files");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_GET_PUBLICKEY_HASH, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_NOT_FOUND},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,\"%%d/.ssh/known_hosts2\"]", .resultInt = SSH_ERROR},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to set '~/.ssh/known_hosts2' known hosts file: ssh_options_set: Out of memory");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - setting secondary global known hosts files");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_NOT_FOUND},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,\"%%d/.ssh/known_hosts2\"]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,\"/etc/ssh/ssh_known_hosts2\"]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "ssh_options_set: Out of memory"},
            {.function = NULL}
        });
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to set '/etc/ssh/ssh_known_hosts2' known hosts file: ssh_options_set: Out of memory");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - unable to reset default known hosts files");
...
//            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,null]", .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to reset default known hosts file: ssh_options_set: Out of memory");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("ssh verify_knownhost - strict - failure - unable to reset default global known hosts files");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SESSION_IS_KNOWN_SERVER, .resultInt = SSH_KNOWN_HOSTS_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[8,null]", .resultInt = SSH_OK},
            {.function = HRNLIBSSH_OPTIONS_SET, .param = "[35,null]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "ssh_options_set: Out of memory"},
            {.function = NULL}
        });
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to reset default global known hosts file: ssh_options_set: Out of memory");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp new - strict - failure");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_PKI_IMPORT_PRIVKEY_FILE, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_USERAUTH_PUBLICKEY, .resultInt = SSH_AUTH_SUCCESS, .param = "[\"" TEST_USER "\"]"},
            {.function = HRNLIBSSH_SFTP_NEW, .resultNull = true},
            {.function = HRNLIBSSH_GET_ERROR_CODE, .resultInt = SSH_FATAL},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "sftp new error"},
            {.function = NULL}
        });
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to allocate sftp session: sftp new error [2]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - strict - failure");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - defaults - success");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - non defaults - success");
        ...
        TEST_RESULT_LOG(
            ...
            "P00   WARN: pgBackRest added new host 'localhost' to '~/.ssh/known_hosts'");
...
//            {.function = HRNLIBSSH_SFTP_INIT, .resultInt = SSH_OK},
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - non defaults - accept-new - ssh_options_set SSH_OPTIONS_KNOWNHOSTS failure");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - accept-new - import pubkey failure");
...
//            {.function = HRNLIBSSH_SFTP_INIT, .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to import public key file '/home/rthompso/.ssh/id_rsa.pub' [-127]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - accept-new - import privkey failure");
...
//            {.function = HRNLIBSSH_SFTP_INIT, .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to import private key file '/home/rthompso/.ssh/id_rsa' [-127]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - accept-new - userauth try publickey failure");
...
//            {.function = HRNLIBSSH_SFTP_INIT, .resultInt = SSH_OK},
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to authenticate with public key: /home/rthompso/.ssh/id_rsa.pub for rthompso [1]");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - accept-new - userauth publickey/privatkey failure");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_PKI_IMPORT_PRIVKEY_FILE, .resultInt = SSH_OK},
            {.function = HRNLIBSSH_USERAUTH_PUBLICKEY, .resultInt = SSH_AUTH_DENIED, .param = "[\"" TEST_USER "\"]"},
            {.function = NULL}
        });
        ...
        TEST_ERROR(
            storageSftpNewP(
                ...
                .knownHosts = strLstNewVarLst(cfgOptionIdxLst(cfgOptRepoSftpKnownHost, repoIdx))),
            ServiceError,
            "unable to authenticate with private key: /home/rthompso/.ssh/id_rsa for user rthompso [1]");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("sftp init - success - SFTP_STRICT_HOSTKEY_CHECKING_NONE");
...
//            "timeout opening directory '" TEST_PATH "/BOGUS'");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            HRNLIBSSH_MACRO_STARTUP(),
            // storagePutP(storageNewWriteP())
            {.function = HRNLIBSSH_SFTP_OPEN, .param = "[\"" TEST_PATH "/.aaa.txt.pgbackrest.tmp\",577,416]", .resultNull = false},
...
//
    ...
    if (testBegin("storagePathCreate()"))
    {
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            HRNLIBSSH_MACRO_STARTUP(),
            // Create /sub1
            {.function = HRNLIBSSH_SFTP_MKDIR, .param = "[\"" TEST_PATH "/sub1\",488]", .resultInt = 0},
            {.function = HRNLIBSSH_SFTP_LSTAT, .param = "[\"" TEST_PATH "/sub1\"]", .resultNull = false,
...
//
            ...
            {.function = HRNLIBSSH_SFTP_READDIR, .fileName = STRDEF("remove2"), .resultNull = NULL,
             ...
             .flags = SSH_FILEXFER_ATTR_PERMISSIONS | SSH_FILEXFER_ATTR_ACMODTIME | SSH_FILEXFER_ATTR_UIDGID,
             .mtime64 = 1656434296, .uid = TEST_USER_ID, .gid = TEST_GROUP_ID},
            {.function = HRNLIBSSH_SFTP_READDIR, .fileName = STRDEF("remove.txt"), .resultInt = 10,
             .attrPerms = SSH_S_IFREG | S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
             .flags = SSH_FILEXFER_ATTR_PERMISSIONS | SSH_FILEXFER_ATTR_ACMODTIME | SSH_FILEXFER_ATTR_UIDGID,
             .mtime64 = 1656434296, .uid = 0, .gid = 0},
            {.function = HRNLIBSSH_SFTP_READDIR, .resultNull = true},
            {.function = HRNLIBSSH_SFTP_DIR_EOF, .resultInt = 1},
            {.function = HRNLIBSSH_SFTP_CLOSEDIR, .resultInt = SSH_NO_ERROR},
//            {.function = HRNLIBSSH2_SFTP_READDIR_EX, .param = "[\"remove.txt\",4095,null,0]", .fileName = STRDEF(""),
...
//            {.function = HRNLIBSSH2_SESSION_BLOCK_DIRECTIONS, .resultInt = SSH2_NO_BLOCK_READING_WRITING},
            ...
            {.function = HRNLIBSSH_SFTP_READDIR, .fileName = STRDEF("remove.txt"), .resultNull = false,
             .attrPerms = SSH_S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO,
             .flags = SSH_FILEXFER_ATTR_PERMISSIONS | SSH_FILEXFER_ATTR_ACMODTIME | SSH_FILEXFER_ATTR_UIDGID,
             .mtime64 = 1656434296, .uid = 0, .gid = 0},
            {.function = HRNLIBSSH_SFTP_READDIR, .resultNull = true},
            {.function = HRNLIBSSH_SFTP_DIR_EOF, .resultInt = 1},
...
//
        ...
        TEST_ERROR_FMT(
            storagePathRemoveP(storageTest, pathRemove2, .recurse = true), PathOpenError,
            STORAGE_ERROR_LIST_INFO ": SFTP server: Permission denied libssh err [1] sftp err [3]", strZ(pathRemove2));
        ...
        // Mimic "sudo chmod 755 %s && sudo touch %s && sudo chmod 777 %s", strZ(pathRemove2), strZ(fileRemove), strZ(fileRemove));
        TEST_ERROR_FMT(
            storagePathRemoveP(storageTest, pathRemove1, .recurse = true),
            PathRemoveError,
            STORAGE_ERROR_PATH_REMOVE_FILE ": SFTP server: Permission denied [1] libssh sftp error [3]", strZ(fileRemove));
...
//            {.function = HRNLIBSSH_SFTP_CLOSE_HANDLE, .resultInt = LIBSSH2_ERROR_NONE},
        ...
        TEST_ERROR_FMT(
            ...
            FileMissingError,
            STORAGE_ERROR_READ_MISSING ": SFTP server: No such file [1] libssh sftp error [2]", strZ(fileName));

        // Missing not no such file
        TEST_ERROR_FMT(
            ioReadOpen(storageReadIo(file)),
            FileOpenError,
...
//            {.function = HRNLIBSSH_SFTP_CLOSE_HANDLE, .resultInt = LIBSSH2_ERROR_SFTP_PROTOCOL},
        ...
        TEST_ERROR(
            storageReadSftpClose((StorageReadSftp *)file->driver), FileCloseError,
            "unable to close file '" TEST_PATH "/readtest.txt' after read: libssh sftp error [4]");
...
//        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));
        ...
        TEST_RESULT_BOOL(ioReadOpen(storageReadIo(file)), true, "open file");
        TEST_ERROR(
            storageReadSftp(((StorageReadSftp *)file->driver), outBuffer, false),
            FileReadError,
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH2_ERROR_SFTP_PROTOCOL},
        ...
        TEST_ERROR_FMT(
            ioWriteOpen(storageWriteIo(file)), FileOpenError,
            STORAGE_ERROR_WRITE_OPEN ": SFTP server: Permission denied [1] libssh sftp error [3]", strZ(fileNoPerm));
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH2_ERROR_SFTP_PROTOCOL},
        ...
        TEST_ERROR_FMT(
            ...
            FileMissingError,
            STORAGE_ERROR_WRITE_MISSING ": SFTP server: No such file [1] libssh sftp error [2]", TEST_PATH "/missing");
...
//            {.function = HRNLIBSSH_SESSION_BLOCK_DIRECTIONS, .resultInt = SSH2_NO_BLOCK_READING_WRITING},
            ...
            {.function = HRNLIBSSH_SFTP_RENAME,
             .param = "[\"" TEST_PATH "/sub1/testfile.pgbackrest.tmp\",\"" TEST_PATH "/sub1/testfile\"]", .resultInt = SSH_FX_OK},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
...
//            {.function = HRNLIBSSH_GET_ERROR_CODE, .resultInt = SSH_FATAL},
        ...
        TEST_ERROR_FMT(
            ioWriteClose(storageWriteIo(file)),
            FileSyncError,
            "unable to sync file '%s' after write: SFTP server: Out of memory libssh sftp error [4]", strZ(fileNameTmp));
...
//            {.function = HRNLIBSSH_SESSION_BLOCK_DIRECTIONS, .resultInt = SSH2_BLOCK_READING},
        ...
        TEST_ERROR_FMT(
            ioWriteClose(storageWriteIo(file)), FileCloseError,
            "unable to close file '%s' after write: SFTP server: Connection lost libssh sftp error [7]", strZ(fileNameTmp));
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH2_ERROR_SFTP_PROTOCOL},
            ...
            {.function = HRNLIBSSH_SFTP_CLOSE, .resultInt = 0},
            {.function = HRNLIBSSH_SFTP_RENAME,
             .param = "[\"" TEST_PATH "/sub1/testfile.pgbackrest.tmp\",\"" TEST_PATH "/sub1/testfile\"]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_CONNECTION_LOST},
            {.function = HRNLIBSSH_GET_ERROR_CODE, .resultInt = SSH_FATAL},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "SFTP server: Connection lost"},
            HRNLIBSSH_MACRO_SHUTDOWN()
        ...
        TEST_ERROR_FMT(
            ioWriteClose(storageWriteIo(file)), FileCloseError,
            "unable to move '%s' to '%s': SFTP server: Connection lost [2] libssh sftp error [7]", strZ(fileNameTmp),
            strZ(fileName));

        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("error on rename - file exists - unable to unlink existing file - SSH_FX_FAILURE");
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH_ERROR_SFTP_PROTOCOL},
            ...
            {.function = HRNLIBSSH_SFTP_CLOSE, .resultInt = SSH_NO_ERROR},
            {.function = HRNLIBSSH_SFTP_RENAME,
             .param = "[\"" TEST_PATH "/sub1/testfile.pgbackrest.tmp\",\"" TEST_PATH "/sub1/testfile\"]",
             .resultInt = -1},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_FAILURE},
            {.function = HRNLIBSSH_SFTP_UNLINK, .param = "[\"" TEST_PATH "/sub1/testfile\"]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_FAILURE},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "SFTP server: Failure"},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        TEST_RESULT_INT(ioWriteFd(storageWriteIo(file)), -1, "check write fd");
        TEST_ERROR_FMT(
            ioWriteClose(storageWriteIo(file)), FileRemoveError,
            "unable to remove existing '%s': SFTP server: Failure libssh sftp error [4]", strZ(fileName));

        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("error on rename - file exists - unable to unlink existing file - SSH_FX_FILE_ALREADY_EXISTS");
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH_ERROR_SFTP_PROTOCOL},
            ...
            {.function = HRNLIBSSH_SFTP_UNLINK, .param = "[\"" TEST_PATH "/sub1/testfile\"]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_FILE_ALREADY_EXISTS},
            {.function = HRNLIBSSH_GET_ERROR, .resultZ = "SFTP server: File already exists"},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        TEST_ERROR_FMT(
            ioWriteClose(storageWriteIo(file)), FileRemoveError,
            "unable to remove existing '%s': SFTP server: File already exists libssh sftp error [11]", strZ(fileName));
...
//            {.function = HRNLIBSSH_SESSION_LAST_ERRNO, .resultInt = LIBSSH_ERROR_SFTP_PROTOCOL},
    ...
    if (testBegin("storagePut() and storageGet()"))
    {
        ...
        TEST_ERROR(
            storageGetP(storageNewReadP(storageTest, TEST_PATH_STR, .ignoreMissing = false)), FileReadError,
            "unable to read '" TEST_PATH "': SFTP server: Received invalid DATA packet from sftp server [2] sftp errno [4]");
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_FAILURE},
            {.function = HRNLIBSSH_SFTP_UNLINK, .param = "[\"" TEST_PATH "/test.empty\"]", .resultInt = SSH_ERROR},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_FAILURE},
        ...
        TEST_ERROR(
            storagePutP(storageNewWriteP(storageTest, emptyFile), NULL), FileRemoveError,
            "unable to remove existing '" TEST_PATH "/test.empty': Failure libssh sftp error [4]");

        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("put - empty file, remove already existing file, storageWriteSftpRename failure");
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("put - empty file, remove already existing file, storageWriteSftpRename success");
...
//            {.function = HRNLIBSSH_SFTP_CLOSE, .resultInt = SSH_NO_ERROR},
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SFTP_OPEN, .param = "[\"" TEST_PATH "/test.txt.pgbackrest.tmp\",577,416]"},
            // Not passing buffer param, see shim function, initial passed buffer contains random data
            {.function = HRNLIBSSH_SFTP_WRITE, .param = "[\"TE\",2]", .resultInt = 2},
...
//            {.function = HRNLIBSSH_SFTP_WRITE, .param = "[1]", .resultInt = 1},
            ...
            {.function = HRNLIBSSH_SFTP_CLOSE, .resultInt = SSH_NO_ERROR},
            {.function = HRNLIBSSH_SFTP_RENAME,
             .param = "[\"" TEST_PATH "/test.txt.pgbackrest.tmp\",\"" TEST_PATH "/test.txt\"]",
             .resultInt = SSH_NO_ERROR},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SFTP_READ, .param = "[3]", .resultInt = 0},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SFTP_READ, .param = "[2]", .resultInt = 2, .readBuffer = STRDEF("LE")},
            {.function = HRNLIBSSH_SFTP_READ, .param = "[2]", .resultInt = 1, .readBuffer = STRDEF("\n")},
            {.function = HRNLIBSSH_SFTP_READ, .param = "[1]", .resultInt = 0},
            {.function = HRNLIBSSH_SFTP_CLOSE},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        TEST_ERROR(
            storageGetP(storageNewReadP(storageTest, STRDEF(TEST_PATH "/test.txt"), .offset = 18446744073709)), FileOpenError,
            "unable to seek to 18446744073709 in file '" TEST_PATH "/test.txt'");
        ...
        TEST_ERROR(
            storageGetP(storageNewReadP(storageTest, STRDEF(TEST_PATH "/test.txt"))), FileReadError,
            "unable to read '" TEST_PATH "/test.txt': SFTP server: Received message 102 during read [2] sftp errno [5]");
...
//        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));
    ...
    if (testBegin("storageRemove()"))
    {
        ...
        {
            HRNLIBSSH_MACRO_STARTUP(),
            {.function = HRNLIBSSH_SFTP_UNLINK, .param = "[\"/missing\"]", .resultInt = -1},
            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultInt = SSH_FX_NO_SUCH_FILE},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("remove - file missing, .errorOnMissing = true, sftp error SSH_FX_NO_SUCH_FILE");
...
//            {.function = HRNLIBSSH_GET_ERROR_CODE, .resultInt = SSH_REQUEST_DENIED},
        ...
        TEST_ERROR(storageRemoveP(storageTest, STRDEF("missing"), .errorOnMissing = true), FileRemoveError,
                   "unable to remove '/missing': SFTP server: No such file libssh err [1] sftp err [2]");

        memContextFree(objMemContext((StorageSftp *)storageDriver(storageTest)));

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("remove - file missing, sftp error other than SSH_FX_NO_SUCH_FILE");
        ...
        TEST_ERROR(storageRemoveP(storageTest, STRDEF("missing")), FileRemoveError,
                   "unable to remove '/missing': SFTP server: Connection lost libssh err [1] sftp err [7]");
...
//            {.function = HRNLIBSSH_SFTP_GET_ERROR, .resultUInt = SSH_FX_NO_SUCH_FILE},
        ...
        TEST_ERROR_FMT(
            ioReadOpen(storageReadIo(file)),
            FileOpenError,
            STORAGE_ERROR_READ_OPEN ": SFTP server: Permission denied [1] libssh sftp error [3]", strZ(fileNoPerm));
        ...
        hrnLibSshScriptSet((HrnLibSsh [])
        {
            ...
            {.function = HRNLIBSSH_SFTP_READ, .param = "[40]", .resultInt = 2, .readBuffer = STRDEF("FI")},
            {.function = HRNLIBSSH_SFTP_READ, .param = "[38]", .resultInt = 2, .readBuffer = STRDEF("LE")},
            {.function = HRNLIBSSH_SFTP_READ, .param = "[36]", .resultInt = 1, .readBuffer = STRDEF("\n")},
            {.function = HRNLIBSSH_SFTP_READ, .param = "[35]", .resultInt = 0},
            {.function = HRNLIBSSH_SFTP_CLOSE},
            HRNLIBSSH_MACRO_SHUTDOWN()
        });
        StringList *argList = strLstNew();
