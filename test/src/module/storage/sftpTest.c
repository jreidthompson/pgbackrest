...
/***********************************************************************************************************************************
Test function for build without libssh2 library
***********************************************************************************************************************************/
static void
storageSftpNotPresent(void)
{
    FUNCTION_TEST_BEGIN();
    FUNCTION_TEST_END();

    THROW_FMT(OptionInvalidValueError, PROJECT_NAME " not built with sftp support");
}
...
testRun(void)
{
    ...
    if (testBegin("storageNew()"))
    {
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("excercise storageSftpNotPresent");

        TEST_ERROR(storageSftpNotPresent(), OptionInvalidValueError,  "pgBackRest not built with sftp support");
#else
        TEST_ERROR(storageSftpNotPresent(), OptionInvalidValueError,  "pgBackRest not built with sftp support");
#endif // HAVE_LIBSSH2
    }
